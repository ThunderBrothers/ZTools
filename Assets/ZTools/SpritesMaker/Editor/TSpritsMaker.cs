...
namespace ZTools.SpritsMaker
{
    public class SpritsMaker : EditorWindow
    {
        ...
        private static FileStructure _fileStructure = FileStructure.单个物体多个动画;//文件目录结构
        ...
        {
            ImagePath = Application.dataPath + "/ZTools/SpritesMaker/Texture";
            AnimationPath = Application.dataPath + "/ZTools/SpritesMaker/Animations";
            AnimationControllerPath = "Assets/ZTools/SpritesMaker/AnimalContrller";
            PrefabPath = "Assets/ZTools/SpritesMaker/Prefabs";
        }
        private void OnGUI()
        {
        ...
        void Reset()
        {
            ...
            hasSelected = false;
            frames = 30;
            _fileStructure = FileStructure.单个物体多个动画;
        }
        //每次创建后重置
        ...
        static void BuildAniamtion()
        {
            ...
            {
                case FileStructure.单个物体多个动画:
                    foreach (DirectoryInfo dictorys in raw.GetDirectories())
                    {
                        tempClip = BuildAnimationClip(dictorys);
                        if (tempClip != null)
                        {
                            clips.Add(tempClip);
                        }
                    ...
                    controller = BuildAnimationController(clips, raw.Name);
                    //最后生成程序用的Prefab文件
                    BuildPrefab(raw, controller);
                    clips.Clear();
                    break;
                case FileStructure.多个物体多动画:
                    foreach (DirectoryInfo dictorys in raw.GetDirectories())
                    {
                        foreach (DirectoryInfo dictoryAnimations in dictorys.GetDirectories())
                        {
                            //每个文件夹就是一组帧动画，这里把每个文件夹下的所有图片生成出一个动画文件
                            tempClip = BuildAnimationClip(dictoryAnimations);
                            if (tempClip != null)
                            {
                                clips.Add(tempClip);
                            }
        ...
        static AnimationClip BuildAnimationClip(DirectoryInfo dictorys)
        {
            ...
            if (images.Length == 0)
            {
                //EditorUtility.DisplayDialog("警告", $"{dictorys.Name}文件夹里有无效文件夹 自动跳过 自己检查有没有少的", "我要重新检查生成个数");
                return null;
            }
            ...
            //有些动画我希望天生它就动画循环
            if (animationName.IndexOf("原地跳起") >= 0)
            {
                //设置idle文件为循环动画
        ...
        static AnimatorController BuildAnimationController(List<AnimationClip> clips, string name)
        {
            ...
            foreach (AnimationClip newClip in clips)
            {
                ...
                if (newClip.name == "原地跳起")
                {
                    sm.defaultState = state;
                }
                else
                {
                    animatorController.AddParameter(newClip.name, AnimatorControllerParameterType.Bool);
                }
            }
            ChildAnimatorState[] childAnimatorStates = sm.states;
            ChildAnimatorState childAnimatorState;
            AnimatorStateTransition animatorStateTransition;
            for (int j = 0;j < childAnimatorStates.Length;j++)
            {
                childAnimatorState = childAnimatorStates[j];
                if (childAnimatorState.state != null && childAnimatorState.state != sm.defaultState)
                {
                    animatorStateTransition = sm.defaultState.AddTransition(childAnimatorState.state);
                    animatorStateTransition.hasExitTime = true;
                    animatorStateTransition.AddCondition(AnimatorConditionMode.If, 0, childAnimatorState.state.motion.name);
                }
            }
            AssetDatabase.SaveAssets();
        ...
        /// <summary>
        /// 这里根据.anim文件使用文本工具打开后内有m_AnimationClipSettings的字段
        /// 若要修改动画的一些查数可以查看m_AnimationClipSettings内容
        /// </summary>
